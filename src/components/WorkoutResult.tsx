    ...
    const txt = [
      ...
      ...treino.dias.map((d) =>
        [
          ...
          ...d.exercicios.map(
            (e) =>
              `- ${e.nome} — ${e.series}x${e.reps} (${e.descanso})${e.observacao ? ` — ${e.observacao}` : ""}`,
          ),
    ...
    <div>
      <div className="relative overflow-hidden rounded-3xl bg-navy p-6 text-cream sm:p-8 md:p-10">
        ...
        <div className="relative mt-6 flex flex-wrap gap-2">
          <Badge icon={<Dumbbell className="h-3.5 w-3.5" />}>
            {t("gerar.days_per_week", { n: treino.diasPorSemana })}
          </Badge>
          <Badge icon={<Clock className="h-3.5 w-3.5" />}>
            {t("gerar.min_per_session", { n: treino.duracaoMin })}
          </Badge>
        </div>
      </div>
      ...
      <div className="no-print mt-6 flex flex-wrap gap-2">
        {onBack && (
          <button
            onClick={onBack}
            className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-5 py-2.5 text-sm font-semibold transition hover:bg-foreground/5"
          >
        ...
        <div className="mt-8 rounded-3xl bg-secondary/30 p-7 print-tips print:bg-white print:p-0">
          ...
          <ul className="mt-3 space-y-2 text-foreground/90">
            {treino.dicas.map((d, i) => (
              <li key={i} className="flex gap-2 print-avoid-break">
    ...
    <div className={`rounded-3xl bg-card p-7 shadow-sm md:p-8 ${className}`}>
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <h2 className="font-display text-2xl">
          {t("gerar.day_label", { letter: String.fromCharCode(65 + index) })} — {dia.nome}
        </h2>
      </div>
      <p className="mt-3 text-sm text-muted-foreground">
        <span className="font-semibold text-foreground">{t("gerar.warmup")}:</span>{" "}
        {dia.aquecimento}
      </p>
      ...
      <p className="mt-5 text-sm text-muted-foreground">
        <span className="font-semibold text-foreground">{t("gerar.stretch")}:</span>{" "}
        {dia.alongamento}
      </p>
    </div>
  );
